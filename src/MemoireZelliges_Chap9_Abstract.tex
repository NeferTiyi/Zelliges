...
\begin{tikzpicture}[
  ...
  \node at (current page.north west) 
        [right, text width=\textwidth, 
         shift={(\hoffset+1in+\evensidemargin, -2cm)}] {%
    \large
    \SL

    % \medskip
    DESS Méthodes Physiques Appliquées à l'Archéologie et à la 
    Muséographie -- Juin 2000\par%
  } ;
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \node at (current page.south west) 
        [right, text width=\textwidth, 
         shift={(\hoffset+1in+\evensidemargin, 3cm)}] 
  {%
    \large
    \begin{motsclef}
       zellige, céramique glaçurée, architecture, \PaM, Meknès, 
       Maroc, \siecle{17}, patrimoine culturel, méthodes physiques 
       de caractérisation
    \end{motsclef}
  } ;
\end{tikzpicture}
